...
void Triangulation<Traits>::init(Iterator begin, Iterator end) {
  ...
  S_0->orientConstraint(m_deref(S_0->vertices[0]), SimplexBase_t::INSIDE);
  ...
  for (int i = 0; i < m_ndim + 1; i++) {
    ...
    S[i]->orientConstraint(m_deref(S_0->vertices[i]), SimplexBase_t::OUTSIDE);
  }
...
void Triangulation<Traits>::find_x_visible(PointRef x, Simplex* S) {
  ...
  if (!S->isVisible(m_deref(x))) S = S->neighbors[0];
  ...
  while (m_xv_walk.size() > 0) {
    ...
    for (int i = 1; i < m_ndim + 1; i++) {
      ...
      if (N->isVisible(m_deref(x)) && !m_xv_walked.isMember(N)) {
        ...
        Scalar d = (m_deref(x) - m_deref(N->vertices[0])).squaredNorm();
        m_xv_walked.push(N);
  ...
  while (m_xv_queue.size() > 0) {
    ...
    for (int i = 1; i < m_ndim + 1; i++) {
      ...
      if (N->isVisible(m_deref(x)) && N->isInfinite(m_antiOrigin) &&
          !m_x_visible.isMember(N) && !m_xv_queue.isMember(N)) {
        m_xv_queue.push(N);
      }
    }
  }
...
void Triangulation<Traits>::alter_x_visible(PointRef x) {
  ...
  for (size_t i = 0; i < nHorizon; i++) {
    ...
    simplex->orientConstraint(m_deref(p_excluded), SimplexBase_t::OUTSIDE);
  }
