 ...
 */
void StringToArgv(const std::string& str, int* argc, char*** argv) {
  std::list<std::string> args = StringToArgv(str);

  *argv = (char**)std::malloc(args.size() * sizeof(char*));

  int i = 0;
  for (std::list<std::string>::iterator it = args.begin(); it != args.end();
       ++it, ++i) {
    std::string arg = *it;
...
std::list<std::string> StringToArgv(const std::string& args) {
  std::stringstream ain(args);     // used to iterate over input string
  ain >> std::noskipws;            // do not skip white spaces
  std::list<std::string> oargs;  // output list of arguments
