 ...
 */
/**
 *  \file   mpblocks/dubins/curves/SolutionRLRb.hpp
 *
 *  \date   Oct 30, 2012
...
#ifndef MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RLR_B_HPP_
#define MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RLR_B_HPP_
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RLRb,Format_t>::solve(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r )
{
    ...
    Vector2d_t A = normalize( mktmp(c[0]-c[1]) );
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RLRb,Format_t>::solve_debug(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r,
            DebugResult& out )
