 ...
 */
/**
 *  \file   mpblocks/dubins/curves/SolutionRLRa.hpp
 *
 *  \date   Oct 30, 2012
...
#ifndef MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RLR_A_HPP_
#define MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RLR_A_HPP_
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RLRa,Format_t>::solve(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r )
{
    ...
    Format_t a = -Dispatch::acos( d / (4*r) );
    ...
    // need to break out the temporary A b/c this expression tree is aparently
    // too large for nvcc
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RLRa,Format_t>::solve_debug(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r,
            DebugResult& out )
