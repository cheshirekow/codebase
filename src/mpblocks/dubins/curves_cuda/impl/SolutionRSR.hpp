 ...
 */
/**
 *  \file   mpblocks/dubins/curves/SolutionRSR.hpp
 *
 *  \date   Oct 30, 2012
...
#ifndef MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RSR_HPP_
#define MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RSR_HPP_
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RSR,Format_t>::solve(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r )
{
...
//        t[i] = c[i] + mktmp(v*r);
    ...
    // point
    l[0] = cwArc( get<2>(q0), a );
    l[1] = cwArc( a, get<2>(q1) );
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RSR,Format_t>::solve_debug(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r,
            DebugResult& out )
