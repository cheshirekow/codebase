...
#ifndef MPBLOCKS_DUBINS_CURVES_EIGEN_HYPER_TWO_THETA_X_LRL_H_
#define MPBLOCKS_DUBINS_CURVES_EIGEN_HYPER_TWO_THETA_X_LRL_H_
...
struct Solver<xSpec, tSpec, LRLa, Format_t> {
  ...
  static Result_t solve(const Vector3d_t& q0, const Hyper_t& h,
    ...
    // calculate the center of the left turn
    const Vector2d_t c0 = leftCenter(q0, r);

    // the goal point (we'll vary the y coordinate)
    Vector3d_t q1;
    q1 << get_constraint<xSpec, 0>(h), q0[0], get_constraint<tSpec, 2>(h);

    // the center of the goal point (again, we'll vary the y coordinate)
    Vector2d_t c2 = leftCenter(q1, r);

    // the arc location of the query and goal points with respect to
    // their circle
    const Format_t alpha0 = leftAngleOf(q0);
    ...
    Format_t arc0 = ccwArc(beta0, beta1);
    Format_t arc1 = cwArc(beta2, beta3);
    Format_t arc2 = ccwArc(beta4, beta5);
    ...
    if (std::abs(dx) + eps < 2 * r) {
      ...
      beta5 = alpha1;
      arc0 = 0;
      arc1 = cwArc(beta2, beta3);
      arc2 = ccwArc(beta4, beta5);
...
#endif  // MPBLOCKS_DUBINS_CURVES_EIGEN_HYPER_TWO_THETA_X_LRL_H_
