...
EigenCairo::EigenCairo(const Cairo::RefPtr<Cairo::Context>& ctx) {
  m_ctx = ctx;
}


void EigenCairo::set_source(const Gdk::RGBA& rgba) {
  m_ctx->set_source_rgba(rgba.get_red(),
                         rgba.get_green(),
                         rgba.get_blue(),
                         rgba.get_alpha());
}
...
void EigenCairo::set_source_rgb(const char* str) {
  ...
  m_ctx->set_source_rgb(spec[0] / 255.0, spec[1] / 255.0, spec[2] / 255.0);
}
...
void EigenCairo::set_source_rgba(const char* str) {
  ...
  m_ctx->set_source_rgba(spec[0] / 255.0, spec[1] / 255.0, spec[2] / 255.0,
                         spec[3] / 255.0);
}

void EigenCairo::user_to_device(Eigen::Matrix<double, 2, 1>& v) const {
  m_ctx->user_to_device(v[0], v[1]);
}
