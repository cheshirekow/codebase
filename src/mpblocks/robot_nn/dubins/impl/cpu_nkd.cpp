    ...
    public Implementation
{
    ...
    NKDImplementation( std::vector<Point>& points, int capacity):
        points(points),
        capacity(capacity)
    {
        result.reserve(k+1);
    ...
    virtual void insert_derived( int i, const Point& x )
    {
        ...
        while( node->child[0] !=0 )
        {
            ...
            for(int j=0; j < 3; j++)
                assert( cell.minExt[j] < cell.maxExt[j]);
                        ...
                        || queue[0].dist < result[0].dist ) )
        {
            ...
            if( node->child[0] == 0 )
            {
                for( int site : node->points )
                {
                    ...
                    for(int i=0; i < 3; i++)
                    {
                        assert( x[i] < pair.hrect.maxExt[i]);
                        assert( x[i] > pair.hrect.minExt[i]);
                    }
...
Implementation* impl_cpu_nkd(std::vector<Point>& points, int capacity )
{
    return new NKDImplementation(points,capacity);
}
