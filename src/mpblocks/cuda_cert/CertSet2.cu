...
void CertSet2::config(int devId)
{
...
int inContact( CertSet2& set, uint_t obj, uint_t cert,
                            const Matrix3f& R0,
                            const Matrix3f& Rv,
                            const Vector3f& T0,
                            const Vector3f& dT,
                            float gamma  )
{
    ...
    set.computeGrid(obj,cert,blocks,threads);

    size_t pitchV  = set.m_pitchV/sizeof(float);
    size_t pitchF  = set.m_pitchF/sizeof(float);

    SetDef vset = set.m_Vsets[obj];
    SetDef fset = set.m_Fsets[cert];
...
//              << def.nF << "\n";
    ...
    // call the kernel
    kernels::check_cert2<<<blocks,threads>>>(
        set.m_g_inV,
        vset.off,
        vset.size,
        pitchV,
    ...
    // retrieve results
    std::vector<int> results(vset.size * fset.size);
    cuda::memcpyT<int>(
            results.data(),
            set.m_g_out,
            results.size(),
...
int inContact_dbg( CertSet2& set, uint_t obj, uint_t cert,
                            const Matrix3f& R0,
                            const Matrix3f& Rv,
                            const Vector3f& T0,
                            const Vector3f& dT,
...
//              << def.nF << "\n";
    ...
    // call the kernel
    kernels::check_cert2_dbg<<<blocks,threads>>>(
        set.m_g_inV,
        vset.off,
        vset.size,
        pitchV,
