...
template <class Traits>
typename Traits::Format_t Distance<Traits>::operator()(const Point_t& pa,
                                                       const Point_t& pb) {
  ...
  Format_t d2a = diff_a[0] * diff_a[0] + diff_a[1] * diff_a[1] +
                 m_weight * diff_a[2] * diff_a[2];

  Format_t d2b = diff_b[0] * diff_b[0] + diff_b[1] * diff_b[1] +
...
template <class Traits>
typename Traits::Format_t Distance<Traits>::operator()(const Point_t& p,
                                                       const Hyper_t& h) {
  Format_t dist2 = 0;
  ...
  if (p[2] < h.minExt[2]) {
    ...
    if (dist2w < dist2i) dist2i = dist2w;
