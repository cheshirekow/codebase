...
KNearestBallCenter<Traits, Allocator>::KNearestBallCenter(const Point_t& center,
                                                          Format_t radius,
                                                          unsigned int k) {
...
template <class Traits, template <class> class Allocator>
void KNearestBallCenter<Traits, Allocator>::reset(const Point_t& center,
                                                  Format_t radius,
                                                  unsigned int k) {
  m_k = k;
  m_center = center;
...
template <class Traits, template <class> class Allocator>
void KNearestBallCenter<Traits, Allocator>::evaluate(const Point_t& q,
                                                     const Point_t& p,
                                                     Node_t* n) {
...
bool KNearestBallCenter<Traits, Allocator>::shouldRecurse(
  ...
  bool isCloser = m_queue.size() > 0 && (dist2 < m_queue.rbegin()->d2);
