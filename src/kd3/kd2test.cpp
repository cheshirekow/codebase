...
struct IsLeafFn {
  bool operator()(Node* node) const { return (node->child[0] == 0); }
};

struct IdxFn {
  int operator()(Node* node) const { return node->idx; }
};

struct ValueFn {
  TFMT operator()(Node* node) const { return node->value; }
};
...
struct LeafInsertFn {
  void operator()(Node* node, int point) const {
    ...
    if (node->points.size() > CAPACITY) {
      ...
      for (int i = 0; i < 2; i++) node->child[i]->points.reserve(CAPACITY + 1);
...
struct ChildFn {
  Node* operator()(Node* node, int which) const { return node->child[which]; }
};

struct PointGetFn {
  TFMT operator()(int point, int i) const { return g_points[point][i]; }
};
...
struct PriorityNode {
  ...
  PriorityNode() : dist(0), node(0) {}

  PriorityNode(TFMT dist, Node* node, const HyperRect& hrect)
      : dist(dist), node(node), hrect(hrect) {
    assert(node);
  }
...
struct ResultNode {
  ...
  ResultNode() : dist(0), point(0) {}

  ResultNode(TFMT dist, int point) : dist(dist), point(point) {}
...
struct QueueMinKeyFn {
  TFMT operator()(const Queue& q) const { return q.front().dist; }
};

struct QueuePopMinFn {
  PriorityNode operator()(Queue& q) const {
    std::pop_heap(q.begin(), q.end(), typename PriorityNode::GreaterThan());
...
struct QueueInsertFn {
  void operator()(Queue& q, TFMT dist, const PriorityNode& pair) const {
    ...
    std::push_heap(q.begin(), q.end(), typename PriorityNode::GreaterThan());
  }
};

struct QueueSizeFn {
  size_t operator()(const Queue& q) const { return q.size(); }
};
...
struct SetSizeFn {
  size_t operator()(const ResultSet& r) const { return r.size(); }
};
...
struct SitesFn {
  std::vector<int>& operator()(Node* node) const { return node->points; }
};
...
struct NodeFn {
  Node* operator()(const PriorityNode& node) const { return node.node; }
};
...
int main() {
  ...
  for (unsigned int i = 0; i < NTEST; i++) {
    for (unsigned int j = 0; j < NDIM; j++) {
      ...
      if (point[j] > bounds.data[MAX][j]) bounds.data[MAX][j] = point[j];
      if (point[j] < bounds.data[MIN][j]) bounds.data[MIN][j] = point[j];
    }
    ...
    kd2::insert(kd2_root, i, IsLeafFn(), LeafInsertFn(), IdxFn(), ValueFn(),
                ChildFn(), PointGetFn());
  }
  ...
  for (unsigned int i = 0; i < NTEST; i++) {
    ...
    findNearest(ePoint, 1, q, r, QueueMinKeyFn(), QueuePopMinFn(),
                QueueInsertFn(), QueueSizeFn(), SetMaxKeyFn(), SetPopMaxFn(),
                SetInsertFn(), SetSizeFn(), PointDistFn(), CellDistFn(),
                IsLeafFn(), ChildrenFn(), SitesFn(), CellFn(), NodeFn());
  ...
  for (unsigned int i = 0; i < NTEST; i++) {
    ...
    for (unsigned int k = 2; k < NTEST; k += 2) {
      ...
      q.reserve(g_nodes.size());
      q.push_back(PriorityNode(0, &g_nodes[0], bounds));
      findNearest(ePoint, k, q, r, QueueMinKeyFn(), QueuePopMinFn(),
                  QueueInsertFn(), QueueSizeFn(), SetMaxKeyFn(), SetPopMaxFn(),
                  SetInsertFn(), SetSizeFn(), PointDistFn(), CellDistFn(),
                  IsLeafFn(), ChildrenFn(), SitesFn(), CellFn(), NodeFn());
...
#endif
      bool passed = true;
      if (kd_res_size(kdResult) != r.size()) passed = false;
