...
// Child was stopped due to one of the ptrace events
struct PtraceStopped : Stopped {
  int ptrace_event;
};

// Child is stopped due to a regular signal delivered to that thread. Note that
// the kernel may deliver the signal to an arbitrary thread within a
// multithreaded process.
struct SignalDeliveryStopped : PtraceStopped {};

// All threads enter this state when a (possibly multithreaded) process receives
// a stopping signal. Stopping signals are only the four: SIGSTOP, SIGTSTP, SIGTTIN, SIGTTOU
struct GroupStopped : PtraceStopped {};

struct PTraceEventStopped : PTraceStopped {};
...
std::string WaitStatusToString(int wstatus) {
  if (WIFEXITED(wstatus)) {
    ...
    return fmt::format("Signaled: {}, coredumped: {}", WTERMSIG(wstatus),
                       WCOREDUMP(wstatus));
