 ...
 */
#ifndef MPBLOCKS_LINALG_ROTATION_CONVERSIONS_H_
#define MPBLOCKS_LINALG_ROTATION_CONVERSIONS_H_
...
void AxisAngleToQuaternion(const linalg::_RValue<Scalar, Exp>& axis,
...
void QuaternionToAxisAngle(const Quaternion<Scalar>& q,
                           linalg::_LValue<Scalar, Exp>* axis, Scalar* angle) {
  assert(axis->size() >= 3);
  Scalar sin_theta_over_2 = linalg::Norm(linalg::Rows(q, 1, 3));
  ...
  *angle = Scalar(2.0) * std::atan2(sin_theta_over_2, cos_theta_over_2);
  ...
  if (*angle < 1e-9) {
    *axis = Scalar(1.0) / sin_theta_over_2 * linalg::Rows(q, 1, 3);
  } else {
    *axis = linalg::Rows(q, 1, 3);
  }
}

template <typename Scalar, class Exp>
void QuaternionToRotationMatrix(const Quaternion<Scalar>& q,
                                linalg::_LValue<Scalar, Exp>* R) {
  assert(R->size() >= 9);
  assert(R->rows() >= 3);
  assert(R->cols() >= 3);
...
template <typename Scalar, class Exp>
void RotationMatrixToQuaternion(const linalg::_RValue<Scalar, Exp>& R,
                                Quaternion<Scalar>* q) {
  assert(R.size() >= 9);
  assert(R.rows() >= 3);
  assert(R.cols() >= 3);
...
#endif  // MPBLOCKS_LINALG_ROTATION_CONVERSIONS_H_
