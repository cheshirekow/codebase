 ...
 */
/**
 *  \file   mpblocks/dubins/curves/SolutionRSL.hpp
 *
 *  \date   Oct 30, 2012
...
#ifndef MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RSL_HPP_
#define MPBLOCKS_DUBINS_CURVES_CUDA2_SOLUTION_RSL_HPP_
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RSL,Format_t>::solve(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r )
{
    ...
    b    = clampRadian( get<2>(q0) + (_PI/2) );
    l[0] = cwArc(b,a);

    a    = clampRadian(a + _PI );
    b    = clampRadian( get<2>(q1) - (_PI/2) );
    l[1] = ccwArc(a,b);
...
template <typename Format_t>
__host__ __device__ Result<Format_t> Solver<RSL,Format_t>::solve_debug(
        const Vector3d_t& q0,
        const Vector3d_t& q1,
        const Format_t r,
            DebugResult& out )
