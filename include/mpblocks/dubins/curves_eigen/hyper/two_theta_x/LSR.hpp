...
#ifndef MPBLOCKS_DUBINS_CURVES_EIGEN_HYPER_TWO_THETA_X_LSR_H_
#define MPBLOCKS_DUBINS_CURVES_EIGEN_HYPER_TWO_THETA_X_LSR_H_
...
struct Solver<xSpec, tSpec, LSR, Format_t> {
  ...
  static Result_t solve(const Vector3d_t& q0, const Hyper_t& h,
    ...
    // the center of the goal point (we'll vary the y coordinate)
    ...
    if ((dx > 0 && cosTheta > 0) || (dx < 0 && cosTheta < 0)) {
      d1 = (dx - 2 * r * std::cos(q0[2] - M_PI / 2)) / cosTheta;
      if (d1 > 0) {
        arc0 = 0;
    ...
    /// the solution minimizing L1
    if ((dx > 0 && cosTheta > 0) || (dx < 0 && cosTheta < 0)) {
      d1 = (dx - 2 * r * std::cos(q1[2] - M_PI / 2)) / cosTheta;
      if (d1 > 0) {
        arc0 = ccwArc(q0[2], q1[2]);
        arc1 = 0;
        dist_i = r * (arc0 + arc1) + d1;
...
#endif  // MPBLOCKS_DUBINS_CURVES_EIGEN_HYPER_TWO_THETA_X_LSR_H_
