...
#ifndef MPBLOCKS_DUBINS_CURVES_EIGEN_SOLUTION_LRL_B_HPP_
#define MPBLOCKS_DUBINS_CURVES_EIGEN_SOLUTION_LRL_B_HPP_
...
/// Solves  left turn followed by a right turn followed by a left turn
/**
 *
                  x  x
               x        x
...
struct Solver<LRLb, Scalar> {
  ...
  static Path<Scalar> solve(const Vector3d& q0, const Vector3d& q1,
    ...
    Scalar a0 = leftAngleOf(q0);  //< angle of vector from center to q1
    Scalar a1 = std::atan2(dc[1], dc[0]);  //< angle of that vector
    s[0] = ccwArc(a0, a1);                   //< ccwise distance

    // calculate the arc distance we travel on the middle circle
    s[1] = M_PI + 2 * a;

    // calculate the arc distance we travel on the second circle
    dc = c[1] - c[2];
    a0 = std::atan2(dc[1], dc[0]);  //< vector between centers of circles
    a1 = leftAngleOf(q1);           //< angle of vector from center to q1
    s[2] = ccwArc(a0, a1);          //< ccwise distance
...
#endif  // MPBLOCKS_DUBINS_CURVES_EIGEN_SOLUTION_LRL_B_HPP_
