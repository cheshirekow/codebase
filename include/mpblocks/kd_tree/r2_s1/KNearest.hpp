...
#ifndef MPBLOCKS_KD_TREE_R2_S1_KNEAREST_HPP_
#define MPBLOCKS_KD_TREE_R2_S1_KNEAREST_HPP_
...
template <class Traits,template<class> class Allocator>
void KNearest<Traits,Allocator>::evaluate(
...
template <class Traits,template<class> class Allocator>
bool KNearest<Traits,Allocator>::shouldRecurse(
        const Point_t& q, const HyperRect_t& r )
{
    Format_t d2  = m_dist2Fn(q,r);
    return ( m_queue.size() < m_k || d2 < m_queue.rbegin()->d2 );
}
