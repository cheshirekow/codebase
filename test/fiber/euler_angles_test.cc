...
TEST(EulerAnglesSimpleTest, ManualTests) {
  fiber::Quaterniond q_expected(0, 1, 0, 0);
  fiber::Quaterniond q_actual =
      fiber::euler::Angles<double, 0, 1, 2>(M_PI, 0, 0);
...
::testing::AssertionResult AssertQuaternionInvolution(
    const fiber::euler::Angles<Scalar, Axes...>& euler) {
  fiber::Quaternion<Scalar> q = euler;
  fiber::euler::Angles<Scalar, Axes...> q_euler(q);
...
::testing::AssertionResult AssertQuaternionMatchesEigen(
  ...
  fiber::Quaternion<Scalar> q_from_eigen(q_eigen.w(), q_eigen.x(), q_eigen.y(),
                                          q_eigen.z());
  return AssertMatrixApproxEquality("q(euler)", "q_eigen", "eps", q,
                                    q_from_eigen, 1e-8)
         << "and norm(q): " << fiber::Norm(q) << "\n";
}
...
TYPED_TEST_P(EulerAnglesTest, QuaternionInvolution) {
  fiber::euler::Angles<typename TypeParam::Scalar, TypeParam::_1,
